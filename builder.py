...
def merge_import_statements(import_states: List[str], lib_names: List[str] = []) -> List[str]:
    ...
    for state in import_states:
        if state.startswith("from"):
            ...
            if lib_name in from_import_states_holder.keys():
    ...
    for lib_name in from_import_states_holder.keys():
        imps = list(set(from_import_states_holder[lib_name]))
...
def main():
    ...
    with open(bin, "w", encoding="utf-8") as f:
